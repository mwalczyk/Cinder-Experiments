...
void main()
{
  vs_out.position = ciModelView * ciPosition;
  vs_out.normal = ciNormalMatrix * ciNormal;

  float noiseOffset = uLayer * 100.0;
  vec2 noiseSeed = ciPosition.xy * uNoiseScale + noiseOffset;

#ifdef USE_NOISE
  gl_ClipDistance[0] = noise(ciElapsedSeconds * uNoiseSpeed + noiseSeed);
#else
